...
- **内置模板**已经打包进主程序，首次运行自动释放到文档目录 `Documents\EVE\templates\<模块>\`，进面板即可看到、选中就能导入；
...
### v6.14（2026 年 9 月 24 日）

- 内置模板改放到 `文档\EVE\templates\<模块>\`，不再写在程序目录旁边（程序解压在临时目录、装在 Program Files 都照常可用）
- 升级后自动把程序目录下的旧 `templates` 送进回收站；里面若有你自己加的文件，则原样保留不动

### v6.13（2026 年 9 月 23 日）

- 项目更名为 EVE BOX，界面与目录结构按模块重构
- 其他工具新增：查看聊天记录、配置方案导入、种菜 / 装配 / 总览模板导入、星系间距查询
- 更换程序图标，清理无用设置与代码
- 内置模板打包进主程序，首次运行自动释放到 templates 目录，自动更新不再丢模板
