...
namespace EVEBox.Common.PeiZhi
{
    ...
    /// </summary>
    public class PeiZhiManager
    {
        private const string ConfigFile = "evesync_config.json";
        private YingYongPeiZhi _config;
        private System.Timers.Timer _debounceTimer;
        private bool _pendingSave = false;
        private readonly object _saveLock = new object();

        public YingYongPeiZhi Config => _config;

        public PeiZhiManager()
        {
            Load();
        ...
        public void Load()
        {
            try
            {
                if (File.Exists(ConfigFile))
                {
                    ...
                    _config = JsonSerializer.Deserialize<YingYongPeiZhi>(json) ?? new YingYongPeiZhi();

                    // 如果 CharacterNames 有数据，同步到 JiaoSeHuanCunManager
                    if (_config.CharacterNames != null && _config.CharacterNames.Count > 0)
                    {
                        JiaoSeHuanCunManager.LoadFromDictionary(_config.CharacterNames);
                    }
                }
                else
                {
                    _config = new YingYongPeiZhi();
                }
            }
            catch (Exception)
            {
        ...
        public void FlushSave()
        {
            ...
            try
            {
                var charNames = JiaoSeHuanCunManager.GetAllCachedNames();
