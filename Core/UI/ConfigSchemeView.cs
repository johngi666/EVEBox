...
namespace EVESyncTool.Core.UI
{
    ...
    /// </summary>
    public class ConfigSchemeView : UserControl
    {
        ...
        public ConfigSchemeView(Action<string> addLogCallback)
        {
        ...
        /// 在资源管理器中打开方案文件夹
        /// </summary>
        private void BtnShow_Click(ConfigScheme scheme)
        {
            try
            {
                if (!Directory.Exists(scheme.FolderPath))
                {
                    CustomMessageBox.Show($"方案文件夹不存在 [{scheme.Name}]", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
        ...
        private void BtnCreate_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("无法定位服务器配置根目录", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                CustomMessageBox.Show("方案名称包含非法字符", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                CustomMessageBox.Show($"同名文件夹已存在: {name}", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                CustomMessageBox.Show($"创建失败: {ex.Message}", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
