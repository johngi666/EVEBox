...
{
    public class WenJianLieBiaoService
    {
        ...
        public (List<YongHuWenJianXiang> users, List<JiaoSeWenJianXiang> chars) ScanFolder(string folder)
        {
            var users = new List<YongHuWenJianXiang>();
            var chars = new List<JiaoSeWenJianXiang>();
            ...
            foreach (string file in Directory.GetFiles(folder))
            {
                ...
                if (userMatch.Success)
                {
                    ...
                    users.Add(new YongHuWenJianXiang
                    {
                        FileName = fileName,
                        UserId = userId,
                        FilePath = file,
                ...
                if (charMatch.Success)
                {
                    ...
                    chars.Add(new JiaoSeWenJianXiang
                    {
                        FileName = fileName,
                        CharacterId = charId,
            ...
            Action<int, string> onNameReceived)
        {
            ...
            {
                string cachedName = JiaoSeHuanCunManager.GetCachedName(id);
                if (!string.IsNullOrEmpty(cachedName))
                {
            ...
            for (int i = 0; i < idsToQuery.Count; i += batchSize)
            {
                ...
                foreach (var kvp in batchResults)
                {
                    if (!string.IsNullOrEmpty(kvp.Value))
                    {
                        ...
                        JiaoSeHuanCunManager.SaveName(kvp.Key, kvp.Value);
                    }
                }
            ...
            Action<int, string> onNameReceived)
        {
            ...
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                try
                {
                    var server = FuWuQiXinXi.GetByDisplayName(_currentServer);
    ...
    public class YongHuWenJianXiang
    {
    ...
    public class JiaoSeWenJianXiang
    {
        public string FileName { get; set; } = string.Empty;
    ...
    public class BeiFenWenJianJiaXinXi
    {
        public string Name { get; set; } = string.Empty;
        public string Path { get; set; } = string.Empty;
        public DateTime CreatedAt { get; set; }
